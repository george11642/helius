...
// ---------- tool-call identity (for the loop's consecutive-duplicate guard) ----------

/** Stable JSON with sorted object keys, so key order can't change the identity. */
...
/** Canonical identity for a tool call — same name + same args (any key order) => same key. */
