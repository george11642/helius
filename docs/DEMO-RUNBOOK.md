...
- [ ] Open the **judge URL with `?prewarm=1&demo=1`** appended (e.g. `https://<your-pages-url>/?prewarm=1&demo=1`). `?prewarm=1` pre-loads both model sizes so the tier swap later is instant — **only ever use it on this rehearsed machine** (it holds two models in GPU memory, safe here but not on an unknown laptop). `?demo=1` turns on **DEMO GPS** with the La Luz preset — without it the app uses *real* geolocation, and a desk-bound Mac would honestly report "no fix" instead of the rehearsed demo scene. `?demo=1` also **skips the first-run onboarding screen** (region-pack picker + capability check + explicit "Load the AI" button) that a plain URL shows — the demo path autoloads immediately, exactly like before. If a judge opens the **plain URL** on their own machine, that onboarding gate is what they'll see: it's intentional (no 3.4 GB download without consent), and a machine that already has the model cached skips straight through it.
...
- [ ] Bottom-left **"⌖ GPS · DEMO" chip → "DEMO MODE — SIMULATED GPS"** panel → confirm **"La Luz upper switchbacks (default)"** is selected and the coords line shows **DEMO GPS**. (`?demo=1` applies it automatically — the chip glows amber and reads "· DEMO" when active; picking any preset also switches to demo GPS. In the field, real phone GPS is the primary source — demo mode is an explicit, first-class feature.) A green **position chip** on the map's top-left mirrors the last `locate()` result, including coverage honesty ("in coverage" / "X km off coverage").
...
| **Route says "off_network"** | Your simulated fix is too far from a trail. Open the **"⌖ GPS" chip** (bottom-left) → pick **"La Luz upper switchbacks (default)"** → re-ask. The map's position chip turns red when you're off coverage — that's the tool being honest, not a bug. | *"Let me drop back onto a mapped trailhead."* |
